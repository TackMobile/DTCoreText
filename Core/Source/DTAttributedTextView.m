...
- (NSArray *)getAccessibleElements
{
	if (!accessibleElements)
	{
		...
		for (NSString *key in self.attributedTextContentView.customViewsForLinksIndex) {
			UIView *linkView = [self.attributedTextContentView.customViewsForLinksIndex objectForKey:key];
